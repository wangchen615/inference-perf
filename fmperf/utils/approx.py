...
def _yield_comparisons(self, supr, actual):
    for act, expected in supr._yield_comparisons(actual):
        for a, e in _yield_nested(act, expected, rel=self.rel, abs=self.abs, nan_ok=self.nan_ok):
            yield a, e


def is_seq_like(obj):
    return hasattr(obj, "__getitem__") and isinstance(obj, Sized) and not isinstance(obj, (bytes, str))
